...
##########################################################################################################
##########################################################################################################
# NEW CODE (the entire file) FOR THE EXENTION: Increment and save Second Player's Score
##########################################################################################################
##########################################################################################################
	...
	.text
	.globl IncrementScore_2, FinalScore_2
...
IncrementScore_2:
	...
	# {(0;0 Points), (1;5 Points), (3;15 Points)...(20;100 Points)}
...
PrintZero:	
	...
	li 	$t1, 27				# load coords (27, 0) 00(0)
	li 	$t2, 0
...
##########################################################################################################
	...
	# Function that increments the Tens 0(0)0 of the score
AddTens:
	...
	li	$t1, 26				# load coords (26, 0) 0(0)0
	li	$t2, 0
...
loadUnits:
	lw	$t1, 0xffff0008			# Load ready bit adress
	andi	$t2, $t1, 1			# Get ready bit value
	beqz	$t2, loadUnits			# If ready bit is not set, loop back
...
loadTens:
	lw	$t1, 0xffff0008			# Load ready bit adress
	andi	$t2, $t1, 1			# Get ready bit value
	beqz	$t2, loadTens			# If ready bit is not set, loop back
...
printHundreds:	
	...
	li 	$t1, 25				# load coords (25, 0) for (0)00
	li 	$t2, 0
...
loadHund:
	lw	$t1, 0xffff0008			# Load ready bit adress
	andi	$t2, $t1, 1			# Get ready bit value
	beqz	$t2, loadHund			# If ready bit is not set, loop back
