...
# Author: Gines Moratalla || https://github.com/ginesmoratalla
	...
	# NEW CODE FOR THE EXENTION: Second Player row Position
	currentPlayer_y_2:	.word 	0
	# NEW CODE FOR THE EXENTION: Second Player row Position
...
RewardPrint:
	...
	# NEW CODE FOR THE EXENTION: Load both Player's row from the stack
	...
	sw	$s2, currentPlayer_y_2		# Both players current rows
	# NEW CODE FOR THE EXENTION: Load both Player's row from the stack

RewardGenerate:

	lw	$t0, currentPlayer_y		# Pull both values into our variables
	lw	$t1, currentPlayer_y_2		# Both players current rows
	...
	# NEW CODE FOR THE EXENTION: Compare current player's row with the random coordinates
	beq	$a0, $t0, RewardGenerate	# If the row of the new reward == current Player 1's row, repeat process
	beq	$a0, $t1, RewardGenerate	# If the row of the new reward == current Player 2's row, repeat process
	# NEW CODE FOR THE EXENTION: Compare current player's row with the random coordinates
